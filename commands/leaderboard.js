module.exports = {
	...
	async execute(context, client, database) {
        ...
        const users = await database.User.getAllUsersByServer(context.message.guild.id);
        const sortedUsers = users.sort((a, b) => b.dataValues.levels.level - a.dataValues.levels.level);
        ...
        sortedUsers.forEach(async (user, index) => {
            ...
            leaderboard = leaderboard + `${index + 1}. ${userInfo.username}#${userInfo.discriminator}\n\tLevel: ${user.dataValues.levels.level}\n\tXP: ${user.dataValues.levels.xp}\n\n`;
